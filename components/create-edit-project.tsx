...
import { Project, UploadedFile } from "@/lib/models";
...
const CreateEditProjectPage = ({ id }: { id?: string }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!id) {
        await projectsAPI.create({ ...formData, images: res });
        toast({
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            ...
            <Select
              value={formData.type}
              onValueChange={(value) =>
                setFormData({ ...formData, type: value as PROJECT_TYPE })
              }
            >
              <SelectTrigger>
              ...
              <SelectContent className="w-full">
                ...
                <SelectItem value={PROJECT_TYPE.BUILDING}>
                  BUILDING
                </SelectItem>
                <SelectItem value={PROJECT_TYPE.INTERIOR}>INTERIOR</SelectItem>
                <SelectItem value={PROJECT_TYPE.OTHERS}>
                  OTHERS
                </SelectItem>
              </SelectContent>
            </Select>
              ...
              {Array.from(selectedImages ?? [])?.map((file, index) => (
                <div key={index} className="relative">
                  ...
                  <button
                    type="button"
                    onClick={() => {
                      const updatedFiles = Array.from(selectedImages ?? []).filter(
                        (_, i) => i !== index
                      );
                      // Convert File[] back to FileList using DataTransfer
